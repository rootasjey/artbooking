...
class CropEditorUtilities {
  const CropEditorUtilities();
  ...
  Future<Uint8List> cropImageDataWithDartLibrary(
    ...
    final Uint8List data = kIsWeb &&
        ...
        /// toByteData does not work on web
    ...
    if (src != null) {
      ...
      src.frames = src.frames.map((Image image) {
        ...
        // appLogger.d('${time3.difference(time2)} : crop/flip/rotate');
    ...
    // it costs a lot of time and blocks the UI.
    // var fileData = encodeJpg(src);

    // Using isolate won't block the UI.
    ...
    if (src != null) {
      ...
      if (kIsWeb) {
        fileData =
            onlyOneFrame ? encodeJpg(src.first) : encodeGifAnimation(src);
      } else {
  ...
  Future<Uint8List?> cropImageDataWithNativeLibrary(
    ...
    // appLogger.d('${DateTime.now().difference(start)} ：total time');
    return result;
  }
